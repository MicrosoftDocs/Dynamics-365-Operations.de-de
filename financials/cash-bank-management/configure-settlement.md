...
Die folgenden Parameter auswirken, z Ausgleiche in Microsoft Dynamics 365 für Arbeitsgänge verarbeitet werden. Ausgleich ist der Prozess für das Begleichen einer Rechnung durch eine Zahlung oder eine Gutschrift. Diese Parameter werden befinden sich im **Ausgleich**-Bereich der **Debitorenparameter** **Kreditorenkontenparameter**-Seiten.
...
-   ** Skontoverwaltung ** – Geben Sie wie eine an [Skonto wird ausgeführt, wenn eine Rechnung wird zu viel bezahlten]( cash-discount-handling-overpayments.md). Eine Überzahlung kann das Skonto verringert werden, kann sie als Differenz behandelt werden, oder sie kann an bleiben für Kreditoren oder Debitoren.
...
-   **Skonti für Gutschriften berechnen** – Legen Sie diese Option auf **Ja** fest, um ein Skonto für Gutschriften automatisch zu berechnen. Bei Debitoren ist eine Gutschriftbuchung eine negative Transaktion, die im Feld **Rechnung** der Seite **Freitextrechnung** einen Wert besitzt, oder eine Rückgabe auf der Seite **Auftrag**.
    -   Die Auswirkungen auf diese Option hängen vom Wert des Felds **Skonto verwenden** auf der Seite **Buchungen ausgleichen** ab. Ist die Option auf festgelegt wird ** Ja **, wird der Rabatt bei, wenn das Verwendungsskonto **** **** Feld auf festgelegt ** Normal **. Wenn das Verwendungsskonto **** **** Feld auf festgelegt wird immer ** **, wird das Skonto immer verwendet, unabhängig von der Einstellung dieses Feld übernommen. Wenn das Verwendungsskonto **** **** Feld auf festgelegt wird nie ** **, wird das Skonto niemals verwendet, unabhängig von der Einstellung dieses Feld übernommen.
...
-   **Ausgleich priorisieren (nur Debitor)** – Legen Sie diese Option auf **Ja** fest, um die Schaltfläche **Nach Priorität markieren** auf den Seiten **Debitorenzahlungen eingeben** und **Buchungen ausgleichen** zu aktivieren. Diese Schaltfläche können Benutzer den zuvor festgelegten Ausgleichsauftrag Buchungen zuweisen.  Nachdem der Ausgleichsauftrag einer Buchung angewendet wurde, können die Auftrags- und Zahlungszuteilung geändert werden, bevor dem Buchen.
-   ** Verwendungspriorität für automatische Ausgleiche ** - Legen Sie diese Option Ja ** ** die festgelegte Prioritätsreihenfolge zu verwenden fest, wenn Buchungen automatisch ausgeglichen werden. Dieses Feld ist nur verfügbar, wenn Sie den Ausgleich ** priorisieren ** ** und automatischer Ausgleich ** Optionen werden auf festgelegt ** Ja **.
