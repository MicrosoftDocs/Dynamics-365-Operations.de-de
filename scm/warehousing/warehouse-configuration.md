...
Die Entitäten, die Sie erstellen, um Ihr Lagerortlayout zu definieren, werden in Abfragen verwendet, die Sie in den Arbeitsvorlagen einrichten, um Arbeitsaufträge am Lagerort zu steuern. Wenn Sie die Zonen, Lagerplatztypen usw. definieren, bedenken Sie, dass verschiedene Bereiche am Lagerort für verschiedene Prozesse verwendet werden. Berücksichtigen Sie ausserdem Faktoren wie physische Merkmale eines bestimmten Bereichs. Beispielsweise könnte es in Bereiche, denen Sie nur einen bestimmten Typ Gabelstapler verwenden können. Oder, wenn Ihr Unternehmen Produktion und anhand derselben Einrichtung hat, sollten Sie einen individuellen Lagerort in Dynamics 365 für Arbeitsgänge erstellen aber andererseits die zwei Arbeitsgänge können, indem Sie Zonengruppen zwei erstellen. Geben Sie die Entitäten beschreibende Name, damit das Nachverfolgen, ist sie zu identifizieren, wenn diese in den Vorlagenabfragen verwenden.
...
Lagerplatzstrumpfunterzeichnungslimit unterstützen, sicherzustellen, dass Arbeit wird nicht erstellt, um anzufordern, dass der Lagerbestand in einen Lagerplatz verschoben werden, von dem nicht die physische Kapazität aufweist, Lagerungs- übernehmen. Wenn Sie beispielsweise einige Lagerplätze in einem Lagerort nur eine Palette pro Ort halten, können Lagerplatzstrumpfunterzeichnungslimit aktiviert werden. ** Menge ** Der Wert kann auf festgelegt werden ** ** 1, Einheit ** ** und der Wert kann auf PL ** ** innerhalb einer bestimmten Lagerplatzprofilgruppierung festgelegt werden. 
...
Wenn Sie die Lagerplätze in einem Lagerort schnell zu erstellen, können Sie den der Position installiert ** ** Assistenten. Im Rahmen dieses Prozesses können Sie das Format der Lagerplatznamen problemlos verwalten.
