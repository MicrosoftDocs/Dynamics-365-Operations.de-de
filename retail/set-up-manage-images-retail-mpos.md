---
title: "Einrichten und Verwalten von modernes für Bilder KleinPOS"
description: "Dieser Artikel werden die Schritte beschrieben, die in Verwaltenbilder für die Einrichtung und andere Entitäten verwendet werden, die in modernem KleinPOS (MPOS) angezeigt werden."
author: MargoC
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="set-up-and-manage-images-for-retail-modern-pos"></a>Einrichten und Verwalten von modernes für Bilder KleinPOS

Dieser Artikel werden die Schritte beschrieben, die in Verwaltenbilder für die Einrichtung und andere Entitäten verwendet werden, die in modernem KleinPOS (MPOS) angezeigt werden.
...
Die Bilder, die in modernem KleinPOS () angezeigt werden müssen MPOS extern gehosteten werden, äußeres Microsoft Dynamics 365 für Einzelhandel - Arbeitsgänge. In der Regel werden sie auf einem Content Management-System, auf einem Content Delivery Network (CDN) oder einem Medienserver gehostet. MPOS ruft dann die Bilder von den entsprechenden Einheiten wie Produkten und Kataloge ab und zeigt sie an, indem es auf die Ziel URL zugreift. Um diese extern gehosteten Bilder abzurufen, braucht MPOS das korrekte URL-Format für die Bilder. Sie können das erforderliche URL-Format für Bilder konfigurieren, indem Sie den Wert für die **medienbasierte URL **im Kanalprofil einrichten und die Funktionen **Medienvorlage definieren **für jede Entität verwenden. Sie können das Format der Standard-URL für eine Teilmenge von Entitäten auch überschreiben, indem Sie dies Funktion **In Excel bearbeiten** verwenden. ** Wichtiger Hinweis: ** In der aktuellen Version von Microsoft Dynamics 365 für Arbeitsgänge, können Sie das URLformat nicht mehr einrichten, indem Sie das Bild ** ** XML für MPOS Attribut in der Attributgruppe ** Standard ** für Entitäten verwenden. Wenn Sie mit Microsoft Dynamics AX 2012 R3 kennen und nun die aktuelle Version von Microsoft Dynamics 365 für Arbeitsgänge verwenden, sollten Sie sicherstellen, ob Sie immer die neuen verwenden ** definieren Sie Medienvorlage ** Funktionen Bilder, um einzurichten. Verwenden oder ändern Sie auf keinen Fall das Attribut **Bild** in der **Standard**-Attributengruppe für Entitäten, einschließlich Produkte. Änderungen, die Sie direkt in der Attributgruppe **Standard** für Bilder machen, werden nicht abgebildet. Diese Option wird in einer späteren Version deaktiviert. In den folgenden Verfahren werden Bilder für die Katalogentität als Beispiel genannte. Diese Verfahren helfen sicherzustellen, dass der korrekte Bildzielpfad implizit für alle Katalogbilder festgelegt wird, die einen allgemeinen Pfad folgen. Wenn Sie beispielsweise einen Medienserver oder ein CDN extern eingerichtet haben und möchten, dass die Bilder in MPOS für eine angegebenen Filiale angezeigt werden, wird Ihnen die Funktionalität **Medienvorlage definieren** helfen, den Pfad festzulegen, von dem MPOS Bilder suchen und holen kann. **Hinweis:** Für dieses Demodatenbeispiel wird der Medienserver im Retail-Server bereitgestellt. Sie können jedoch auf eine beliebige Stelle haben es außerhalb Dynamics 365 für Arbeitsgänge.

### <a name="set-up-the-media-base-url-for-a-channel"></a>Die medienbasierte URL für einen Kanal einrichten

1.  Öffnet das Dynamics 365 für Arbeitsgänge Hauptniederlassungs-Portal.
2.  ** Sie Einzelhandel und Handel ** &gt; ** Kanal Einstellungen ** &gt; ** Kanalprofile **. ![channel-profile1 ([]. /media/channel-profile1.png)]". /media/channel-profile1.png)
3.  Im Kanalprofil, das Ihr Shop für MPOS verwendet, aktualisieren Sie das Feld mit der **Medienbasierten URL** mit der Basis-URL von Ihrem Medienserver oder CDN. Die Basis-URL beträgt der erste Teil der URL, die durch alle Bildordner verschiedener Entitäten freigegeben wird.![channel-profile2 ([]. /media/channel-profile2.png)]". /media/channel-profile2.png)

### <a name="define-the-media-template-for-an-entity"></a>Definieren Sie die Medienvorlage für eine Entität

1.  ** Sie Einzelhandel und Handel ** &gt; ** Katalogverwaltung ** ** &gt; Katalogbilder **.
...
9.  Um die Medienvorlage für Katalogbilder auf der Kanalseite aktualisieren, stellen Sie sicher ** Katalog-Einzelvorgang 1150 ** ** Einzelhandel IT ** ** vom Verteilungs zeitplan &gt; auszuführenden **.![catalog1 ([]. /media/catalog1.png)]". /media/catalog1.png)

## <a name="previewing-an-image-from-the-entity-level"></a>Zeigen Sie ein Bild auf Entitätsebene in der Vorschau an
1.  Über die Seite für den Entitätsartikel in HQ, können Sie das Bild in der Vorschau anzeigen, das die Bild URL verwendet, die von der Medienvorlage geholt wird. Klicken Sie in diesem Beispiel im gewünschten Katalog, und klicken Sie dann im Aktivitätsbereich, fahren auf ** Medien ** &gt; ** Bilder **. Verwenden Sie die Auswahlliste, um verschiedene Filialen auszuwählen, die möglicherweise verschiedene Kanalprofile haben.
...
Für alle fünf unterstützten Entitäten (Arbeiter, Debitor, Katalog, Kategorie Produkte) und, wenn Sie aktualisieren ** definieren Sie Medienvorlage ** Dialogfeld Sie auf, um ein Bild einrichten, überprüfen, ob Sie den Katalogeinzelvorgang " 1150 "von aktivieren ** Einzelhandel IT ** ** Verteilungszeitplan &gt; **. Dieser Einzelvorgang aktiviert die aktualisierte Medienvorlage und sorgt dafür, dass sie im Kanal synchronisiert und von MPOS verwendet wird. Aktivieren des Katalogeinzelvorgang (1150 ), nachdem Sie folgenden Änderungen vornehmen:

-   Aktualisieren Sie die Katalogbild-Medienvorlage von Katalogbilder ** ** ** definieren Medienvorlage &gt; **.
-   Aktualisieren Sie die Mitarbeiterbild-Medienvorlage von Mitarbeiterbilder ** ** ** definieren Medienvorlage &gt; **.
-   Aktualisieren Sie die Debitorenbild-Medienvorlage von Debitorenbild ** ** ** definieren Medienvorlage &gt; **.
-   Aktualisieren Sie die Produktbildmedienvorlage ** von Produktbildern ** ** definieren &gt; Medienvorlage **.
-   Aktualisieren Sie die Kategoriebild-Medienvorlage von Kategoriebilder ** ** ** definieren Medienvorlage &gt; **. Sie müssen den Kanal auch veröffentlichen.
...
7.  Wenn Sie aufgefordert werden, klicken Sie im rechten Bereich auf **Diesem Add-In vertrauen**, um die Installation abzuschließen. [![Vertrauenswürdigkeit das Add-In] ". /media/excel4.jpg)]". /media/excel4.jpg)
8.  Wenn Sie aufgefordert werden, sich anzumelden, geben Sie die diese Anmeldeinformationen ein die Sie verwendeten, um sich bei HQ anzumelden. ![Anmeldungsaufforderung( [] . /media/excel5.png)]". /media/excel5.png)
...
11. Für alle Entitäten außer Produkte können Sie die Bild URL überschreiben. Ändern Sie dies vorhandene Bild URL, damit sie die neue Ziel URL des Bilds verwendet und aktualisieren Sie den Dateinamen mit dem neuen Dateinamen für die Bilddatei. Der Dateiname muss eindeutig sein, um sicherzustellen, dass der Datensatz eindeutig ist. [![setzen die Bild - URLs in Excel] ". /media/excel6.jpg)]". ** Hinweis: /media/excel6.jpg)** Wenn Sie Bild URLs für Produktentitäten überschreiben, indem Sie das in bearbeitet Excel-Funktionen oder in der Entitätsartikelseite verwenden, werden MPOS immer alle Medienvorlagen-Bild URLs sowie die überschriebenen Bild URLs an.
...
1.  ** Auf Kataloge ** &gt; ** Medien ** &gt; ** Bilder ** und wählen Sie das Katalogbild aus, um zu aktualisieren.
...
** [] (cat3![. /media/cat3.png)]". -/media/cat3.png) Hinweis: ** Nach Kategoriebildzuordnung, muss der Kanal veröffentlichen und den Kanaleinzelvorgang aktivieren zu unterstützen, sicherzustellen, dass die Änderungen an der Kanaldatenbank veröffentlicht werden.
...
1.  Produktinformationsverwaltung auf ** ** ** &gt; Produkte ** ** &gt; ** Produkte.
...
1.  Z Katalog, und klicken Sie dann im Aktivitätsbereich, fahren auf ** Medien ** &gt; ** Bilder **.
