...
ms.locfileid: "3019804"
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="cf6f2-103">Ausführen von Bootstrap mit Unternehmensdaten – FAQ</span><span class="sxs-lookup"><span data-stu-id="cf6f2-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]

[!include [preview-banner](../../includes/preview-banner.md)]

## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="cf6f2-104">Warum benötige ich Bootstrapping?</span><span class="sxs-lookup"><span data-stu-id="cf6f2-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="cf6f2-105">Sie verfügen möglicherweise über eine vorhandene Common Data Service- oder andere Dynamics 365-App-Instanz mit Geschäftsdaten und möchten die Verbindung für duales Schreiben für diese aktivieren.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="cf6f2-106">In diesem Fall müssen Sie ein Bootstrap für Daten in Common Data Service oder anderen Dynamics 365-Apps mit Unternehmensinformationen ausführen, bevor Sie die Verbindung für duales Schreiben aktivieren.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="cf6f2-107">Wann sollte Bootstrapping verwendet werden?</span><span class="sxs-lookup"><span data-stu-id="cf6f2-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="cf6f2-108">Sie sollten Bootstrapping verwenden, bevor Sie  Entitätszuordnungen für duales Schreiben aktivieren (in Schritt 5).</span><span class="sxs-lookup"><span data-stu-id="cf6f2-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="cf6f2-109">Um die Verbindung für duales Schreiben zwischen Instanzen der Finance and Operations-App und Common Data Service oder einer anderen Dynamics 365-App einzurichten, melden Sie sich bei der Finance and Operations-App als Administrator an.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="cf6f2-110">Wechseln Sie zum Modul **Datenverwaltung**, und klicken Sie auf die Schaltfläche **Duales Schreiben**.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="cf6f2-111">Dadurch wird **Data Integrator** gestartet.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="cf6f2-112">Stellen Sie die Verbindung für duales Schrieben für mindestens ein Unternehmen her.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="cf6f2-113">![Herstellen der Verbindung für duales Schreiben](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="cf6f2-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="cf6f2-114">Aktivieren Sie die Entitätszuordnung **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="cf6f2-115">Dies synchronisiert Unternehmen aus der Finance and Operations-App zu Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="cf6f2-116">![Aktivieren der Entitätszuordnung](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="cf6f2-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="cf6f2-117">Führen Sie den Beispielbootstrappingcode auf der Common Data Service- oder einer anderen Dynamics 365-App-Instanz aus.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="cf6f2-118">Nachdem das Bootstrapping abgeschlossen und das System für die Live-Synchronisierung bereit ist, aktivieren Sie die Entitätszuordnungen.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="cf6f2-119">Durch die Aktivierung von Entitätszuordnungen wird die Erstsynchronisierung der Daten für die aktivierten Entitätszuordnungen ausgelöst.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="cf6f2-120">Die Daten, die den über die Verbindung für duales Schreiben ausgewählten Unternehmen entsprechen, werden zwischen der Finance and Operations-App und Common Data Service synchronisiert.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="cf6f2-121">Wie verwende ich das Codebeispiel?</span><span class="sxs-lookup"><span data-stu-id="cf6f2-121">How to I use the code sample?</span></span>
<span data-ttu-id="cf6f2-122">Der Beispielcode ist eine C#-Anwendung, die Sie in Visual Studio laden können.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="cf6f2-123">Es werden NuGet-Paketabhängigkeiten im Common Data Service SDK verwendet, die Sie durch standardmäßige Visual Studio-Tools aktualisieren können.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="cf6f2-124">Nachdem Sie die Lösung in Visual Studio entzippt und geöffnet und die NuGet-Pakete wiederhergestellt haben, suchen Sie im Code nach **TODO**.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="cf6f2-125">Jede Entscheidung, die Sie zur Art und Weise der Ausführung von Bootstrapping für Unternehmensinformationen treffen müssen, wird durch ein **TODO** mit Beispielcode für eine kanonische Implementierung gekennzeichnet.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="cf6f2-126">Der Beispielcode zeigt nur eine von vielen Methoden zum Kategorisieren von Entitätsdatensätzen nach Unternehmen.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="cf6f2-127">Wenn Sie die Logik in den **TODO**-Abschnitten ändern, können Sie Ihre benutzerdefinierte Kategorisierung erstellen.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="cf6f2-128">Was sollte ich erwarten?</span><span class="sxs-lookup"><span data-stu-id="cf6f2-128">What should I expect?</span></span>
<span data-ttu-id="cf6f2-129">Mit der Beispielanwendung können Sie standardmäßig ein Wörterbuch mit Zuordnungen zwischen Geschäftseinheiten und Unternehmenscodes bereitstellen.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="cf6f2-130">Jede Entität, für die Sie Bootstrapping mit dem Feld **OwningBusinessUnit** ausführen, wird automatisch für die Verwendung des angegebenen Unternehmens festgelegt.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="cf6f2-131">Jede Entität ohne das Feld **OwningBusinessUnit**, z. B. Produkt, legt das Unternehmen basierend auf der Zuordnung mit einem leeren Unternehmenseinheitswert fest.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="cf6f2-132">Die Konsolenanwendung erwartet einen Parameter, entweder **–simulate** oder **–apply**.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="cf6f2-133">Wenn Sie den Befehlszeilenparameter **– simuliert werden** verwenden, werden keine Daten aktualisiert.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="cf6f2-134">Nur **simulation_<entityname>.csv**-Dateien werden im selben Verzeichnis wie das Tool generiert, eine für jede Entität, die aktualisiert worden wäre.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="cf6f2-135">Sie können diese Dateien während der Arbeit iterativ überprüfen, um sicherzustellen, dass der Code die Unternehmenswerte wie erwartet aktualisiert.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="cf6f2-136">Verwenden Sie den **–apply**-Parameter, wenn Sie die simulierten Aktualisierungen abgeschlossen haben.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="cf6f2-137">Dadurch werden alle Datensätze, die aktuell einen falschen Unternehmenswert aufweisen, in Stapeln von 1000 Datensätzen gleichzeitig (standardmäßig) aktualisiert.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="cf6f2-138">Der Code ist wie angegeben idempotent, d. h. Sie können ihn erneut ausführen, sodass nur die falsch zugeordneten Unternehmen aktualisiert werden.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="cf6f2-139">Bei Ausführung mit **–apply** gibt der Code CSV-Dateien der vorgenommenen Änderungen aus, die als **applied_<entityname>.csv** bezeichnet werden.</span><span class="sxs-lookup"><span data-stu-id="cf6f2-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
